...
## What Comes Next
- FreeDrop **rendering** follow-up: extend `PhantomObserver.render()` to iterate `dropletIds` (still the blocker — W43/W44/W45 state is correct in WASM but invisible to users)
- W46 (gravity) — final FreeDrop ward; extends `FreeParticle::integrate` with `velocity += gravity * dt`

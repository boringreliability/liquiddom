...
- W41 mid-session `device.lost` does NOT auto-rebuild as Canvas2D (v1 deferral); `render()` bails on null device + `setBackgroundTexture` silently no-ops.
- **W62 fix-target blocking next RC.** WebGPU paints FreeDrop as AABB rects because W38 SDF shader has no FreeDrop branch (Canvas2D W56 dispatcher works).
- W61 multi-instance recursion: HANDLED via TS-side defenses (activeCores GC pin, inTick + wasmCallInFlight mutex, bridge rebind on stale views, try/catch fail-stop on tick) + `liquiddom:instance-panic` event for host-driven auto-recovery. A residual wasm-bindgen WasmRefCell race under V8 GC timing still fires occasionally on boot but is now invisible to users — orchestrators (`wireDemoEmbed`, `mountLiveHero`, React/Vue adapter wrappers planned) listen for the event and remount within a few frames.

## What Comes Next
- **W61: core renderer bugs → republish as `0.2.0-rc.1`.** Multi-instance recursion + WebGPU FreeDrop AABB rendering. Both fix-targets before re-publishing.
